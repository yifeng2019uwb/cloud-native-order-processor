...
```bash
docker compose exec redis redis-cli KEYS "ip_block:*"
docker compose exec redis redis-cli KEYS "login_fail:*"
```

Both should print `(empty array)`. Then run your tests immediately.
...
The IP block test triggers the gateway to block **your client IP** for 5 minutes (TTL 300s). All requests from the same machine (auth, user, order, inventory, etc.) go through the gateway from that IP, so they get **403** until the block expires.
...
2. **Wait 5 minutes** for the block TTL to expire, then run other tests.
