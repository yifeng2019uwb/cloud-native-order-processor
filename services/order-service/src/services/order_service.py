...
class OrderService:
    ...
    async def create_order(self, order_data: OrderCreate, db: asyncpg.Connection) -> OrderResponse:
        ...
        for item in order_data.items:
            ...
            # Check inventory
            inventory = await db.fetchrow(InventoryQueries.GET_INVENTORY, item.product_id)
        ...
        async with db.transaction():
            ...
            await db.execute(
                ...
                order_id, customer_id, order_data.customer_email, order_data.customer_name,
                OrderStatus.PENDING, total_amount,
            ...
            for item in order_items:
                ...
                await db.execute(
                    ...
                    item.quantity, datetime.utcnow(), item.product_id
                )
    ...
    async def get_order(self, order_id: str, db: asyncpg.Connection) -> Optional[OrderResponse]:
        """Get order by ID"""
    ...
    async def list_orders(self, db: asyncpg.Connection, limit: int = 50, 
                         status: Optional[str] = None, 
                         customer_email: Optional[str] = None) -> List[OrderResponse]:
    ...
    async def update_order_status(self, order_id: str, new_status: OrderStatus, db: asyncpg.Connection) -> bool:
        """Update order status"""
        result = await db.execute(
            OrderQueries.UPDATE_ORDER_STATUS,
            new_status.value, datetime.utcnow(), order_id
        )
        ...
        if result == "UPDATE 1":
            ...
            if order:
                await self.event_service.publish_order_event('order_status_updated', order)
            return True
