...
Commands:
    ...
    test [test_target]     Run auth service tests (all or specific)
...
check_prerequisites() {
    ...
    if [[ ! -f "src/main.py" ]]; then
        log_error "Not in auth service directory - src/main.py not found"
        exit 1
    fi
...
# Check imports
success, issues = validate_service_imports('auth_service')
if not success:
    for issue in issues:
        print(f'✗ {issue}')
    sys.exit(1)
