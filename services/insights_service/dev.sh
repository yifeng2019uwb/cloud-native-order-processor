#!/bin/bash
# services/insights_service/dev.sh
# Insights Service development script - build, test, clean
...
show_usage() {
    cat << EOF
Insights Service Development Script
...
Commands:
    ...
    test [test_target]     Run insights service tests (all or specific)
    run                    Start the insights service (port 8004)
...
check_prerequisites() {
    ...
    if [[ ! -f "src/main.py" ]]; then
        log_error "Not in insights service directory - src/main.py not found"
        exit 1
    fi
...
install_dependencies() {
    ...
    "$VENV_PYTHON" -m pip install -q -r ../common/requirements.txt
...
# Check imports
success, issues = validate_service_imports('insights_service')
if not success:
    for issue in issues:
        print(f'✗ {issue}')
    sys.exit(1)
...
test() {
    ...
    if [[ -n "$test_target" ]]; then
        ...
        if [[ "$test_target" == *".py" ]]; then
            python -m pytest "tests/$test_target" -v --tb=short --cov=src --cov-report=term-missing --cov-report=html
        else
            python -m pytest "tests/" -k "$test_target" -v --tb=short --cov=src --cov-report=term-missing --cov-report=html
        fi
    else
        log_info "Running all tests with coverage..."
        python -m pytest tests/ -v --tb=short --durations=10 --cov=src --cov-report=term-missing --cov-report=html
    fi
