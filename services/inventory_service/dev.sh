...
Commands:
    ...
    test [test_target]     Run inventory service tests (all or specific)
...
Examples:
    ...
    $0 test test_assets        # Test specific test file
...
check_prerequisites() {
    ...
    if [[ ! -f "src/main.py" ]]; then
        log_error "Not in inventory service directory - src/main.py not found"
        exit 1
    fi
...
install_dependencies() {
    ...
    # Add common requirements (includes pytest)
...
# Check imports
success, issues = validate_service_imports('inventory_service')
if not success:
    for issue in issues:
        print(f'✗ {issue}')
    sys.exit(1)
