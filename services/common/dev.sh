#!/bin/bash
# services/common/dev.sh
# Common Package development script - build, test, clean
...
show_usage() {
    cat << EOF
Common Package Development Script

Usage: $0 {build|test|clean} [test_target]

Commands:
    build              Build the common package
    test [test_target] Run common package tests (all or specific module)
    clean              Clean build artifacts

Examples:
    $0 build                    # Build common package
    $0 test                     # Run all tests
    $0 test logging            # Test specific module (logging)
...
build() {
    ...
    log_success "Common package build completed"
}
...
test() {
    ...
    log_success "Common package tests completed"
}

# Clean build artifacts
clean() {
    log_info "Cleaning common package build artifacts..."
    ...
    if [[ -d "htmlcov-common" ]]; then
        rm -rf htmlcov-common
        log_info "Removed htmlcov-common directory"
    fi

    if [[ -f ".coverage" ]]; then
        rm -f .coverage
        log_info "Removed .coverage file"
    fi

    log_success "Common package cleanup completed"
}
