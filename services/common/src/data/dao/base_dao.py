...
class BaseDAO(ABC):
    ...
    def _safe_get_item(self, table, key: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if item:
                logger.info(
                action=LogActions.DB_OPERATION,
                message=f"Item found: key={key}"
            )
            else:
                logger.info(
                    action=LogActions.DB_OPERATION,
                    message=f"Item not found: key={key}"
                )
            return item  # Return None if item doesn't exist
        except Exception as e:
            logger.error(
                action=LogActions.ERROR,
                message=f"Failed to get item with key {key}: {e}"
            )
    ...
    def _safe_put_item(self, table, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            logger.info(
                action=LogActions.DB_OPERATION,
                message=f"Putting item to DynamoDB: {item}"
            )
            response = table.put_item(Item=item)
            logger.info(
                action=LogActions.DB_OPERATION,
                message=f"Item put successfully: {item}"
            )
            return item
        except Exception as e:
            logger.error(
                action=LogActions.ERROR,
                message=f"Failed to put item: {e}"
            )
    ...
    def _safe_update_item(self, table, key: Dict[str, Any],
        ...
        try:
            logger.info(
                action=LogActions.DB_OPERATION,
                message=f"Updating item in DynamoDB: key={key}, expression={update_expression}, values={expression_values}"
            )
            ...
            logger.info(
                action=LogActions.DB_OPERATION,
                message=f"Item updated successfully: key={key}"
            )
            return attributes
        except Exception as e:
            logger.error(
                action=LogActions.ERROR,
                message=f"Failed to update item with key {key}: {e}"
            )
    ...
    def _safe_delete_item(self, table, key: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(
                action=LogActions.ERROR,
                message=f"Failed to delete item with key {key}: {e}"
            )
