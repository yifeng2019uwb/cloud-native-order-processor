...
def handle_orders(method: str, path: str, query_params: Dict, body: Dict) -> Dict:
    ...
    try:
        if method == "POST" and path == "/orders":
            ...
            method == "PUT" and path.startswith("/orders/") and path.endswith("/status")
        ):
            # Update order status
...
def handle_inventory(method: str, path: str, query_params: Dict, body: Dict) -> Dict:
    ...
    try:
        if method == "GET" and path == "/products":
            ...
            method == "PUT"
            and path.startswith("/inventory/")
            and path.endswith("/stock")
        ):
            # Update stock
    ...
    for item in items:
        if not all(k in item for k in ["product_id", "quantity"]):
            return create_response(
                400, {"error": "Each item must have product_id and quantity"}
            )
        ...
        if not product:
            return create_response(
                400, {"error": f'Product {item["product_id"]} not found'}
            )
...
def process_payment(order_id: str, body: Dict) -> Dict:
    ...
    if float(body["amount"]) != float(order["total_amount"]):
        return create_response(
            400, {"error": "Payment amount does not match order total"}
        )
